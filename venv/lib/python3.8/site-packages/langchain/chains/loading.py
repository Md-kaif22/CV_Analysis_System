...
def _load_reduce_documents_chain(
    config: dict, **kwargs: Any
) -> ReduceDocumentsChain:  # type: ignore[valid-type]
...
def _load_llm_bash_chain(config: dict, **kwargs: Any) -> Any:
    ...
    if llm_chain:
        return LLMBashChain(
            llm_chain=llm_chain, prompt=prompt, **config
...
def _load_qa_with_sources_chain(config: dict, **kwargs: Any) -> QAWithSourcesChain:
    ...
    return QAWithSourcesChain(
        combine_documents_chain=combine_documents_chain, **config
    )  # type: ignore[arg-type]
...
def _load_sql_database_chain(config: dict, **kwargs: Any) -> Any:
    ...
    if "llm_chain" in config:
        ...
        return SQLDatabaseChain(
            llm_chain=chain, database=database, **config
        )  # type: ignore[arg-type]
    if "llm" in config:
        llm_config = config.pop("llm")
        llm = load_llm_from_config(llm_config, **kwargs)
