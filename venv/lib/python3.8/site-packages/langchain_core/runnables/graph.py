...
def node_data_json(
    ...
    if isinstance(node.data, RunnableSerializable):
        ...
        json = {"type": "unknown", "data": node_data_str(node.id, node.data)}
...
class Graph:
    ...
    def reid(self) -> Graph:
        ...
        return Graph(
            ...
            edges=[
                edge.copy(
                    source=_get_node_id(edge.source), target=_get_node_id(edge.target)
                )
                for edge in self.edges
            ],
        )
    ...
    def draw_ascii(self) -> str:
        ...
        return draw_ascii(
            {node.id: node.name for node in self.nodes.values()}, self.edges
        )
