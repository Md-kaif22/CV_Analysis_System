...
def _texts_to_nodes(
    ...
    for text in texts:
        ...
        if not isinstance(links, list):
            links = list(links)
        yield Node(id=_id, metadata=_metadata, text=text, links=links)
...
def _documents_to_nodes(documents: Iterable[Document]) -> Iterator[Node]:
    for doc in documents:
        ...
        if not isinstance(links, list):
            links = list(links)
        yield Node(id=doc.id, metadata=metadata, text=doc.page_content, links=links)
...
def nodes_to_documents(nodes: Iterable[Node]) -> Iterator[Document]:
    ...
    for node in nodes:
        ...
        yield Document(id=node.id, page_content=node.text, metadata=metadata)
...
class GraphVectorStore(VectorStore):
    ...
    @abstractmethod
    def add_nodes(self, nodes: Iterable[Node], **kwargs: Any) -> Iterable[str]:
    ...
    async def aadd_nodes(
        self, nodes: Iterable[Node], **kwargs: Any
    ) -> AsyncIterable[str]:
        """Add nodes to the graph store.
    ...
    def add_documents(self, documents: Iterable[Document], **kwargs: Any) -> List[str]:
    ...
    def traversal_search(
        self, query: str, *, k: int = 4, depth: int = 1, **kwargs: Any
    ) -> Iterable[Document]:
