...
def method_is_overridden(
    instance_or_cls: Union[Type[Any], object], against_method: Callable[..., Any]
) -> bool:
...
@overload
def get_cls_kwargs(
    cls: type, *, _set: Optional[Set[str]] = None, raiseerr: Literal[True] = ...
) -> Set[str]:
    ...
...
def format_argspec_plus(
    ...
    apply_kw = compat.inspect_formatargspec(
        name_args, spec[1], spec[2], defaulted_vals, formatvalue=lambda x: "=" + str(x)
    )
...
def monkeypatch_proxied_specials(
    into_cls, from_cls, skip=None, only=None, name="self.proxy", from_instance=None
):
...
class symbol(int):
    ...
    def __new__(
        cls, name: str, doc: Optional[str] = None, canonical: Optional[int] = None
    ) -> symbol:
        with cls._lock:
