...
class UniqueAppender(Generic[_T]):
    ...
    def __init__(
        self, data: Union[Iterable[_T], Set[_T], List[_T]], via: Optional[str] = None
    ):
        self.data = data
        self._unique = {}
        if via:
...
class LRUCache(typing.MutableMapping[_KT, _VT]):
    ...
    __slots__ = ("capacity", "threshold", "size_alert", "_data", "_counter", "_mutex")
    ...
    def _manage_size(self) -> None:
        ...
        try:
            ...
            while len(self) > self.capacity + self.capacity * self.threshold:
                ...
                by_counter = sorted(
                    self._data.values(), key=operator.itemgetter(2), reverse=True
                )
...
class ScopedRegistry(Generic[_T]):
    ...
    def __call__(self) -> _T:
        ...
        try:
            ...
            return self.registry.setdefault(
                key, self.createfunc()
