...
class _LiteralRoundTripFixture:
    ...
    def literal_round_trip(self, metadata, connection):
        ...
        def run(
            type_, input_, output, filter_=None, compare=None, support_whereclause=True
        ):
            ...
            if support_whereclause and self.supports_whereclause:
                if compare:
                    stmt = t.select().where(
                        t.c.x == literal(compare, type_, literal_execute=True),
                        t.c.x == literal(input_[0], type_, literal_execute=True),
                    )
                else:
                    stmt = t.select().where(
                        t.c.x
...
class ArrayTest(_LiteralRoundTripFixture, fixtures.TablesTest):
    ...
    def test_literal_simple(self, literal_round_trip):
        literal_round_trip(
            ARRAY(Integer), ([1, 2, 3],), ([1, 2, 3],), support_whereclause=False
        )
...
class StringTest(_LiteralRoundTripFixture, fixtures.TestBase):
    ...
    @testing.combinations(
        ("%B%", ["AB", "BC"]), ("A%C", ["AC"]), ("A%C%Z", []), argnames="expr, expected"
    )
    def test_dont_truncate_rightside(self, metadata, connection, expr, expected):
        ...
        eq_(connection.scalars(select(t.c.x).where(t.c.x.like(expr))).all(), expected)
    ...
    def test_concatenate_clauselist(self, connection):
        ...
        """
        eq_(connection.scalar(select(literal("a") + "b" + "c" + "d" + "e")), "abcde")
...
class TrueDivTest(fixtures.TestBase):
    ...
    @testing.combinations(
        ("15", "10", 1.5), ("-15", "10", -1.5), argnames="left, right, expected"
    )
...
class BooleanTest(_LiteralRoundTripFixture, fixtures.TablesTest):
    ...
    def test_whereclause(self):
        ...
        with config.db.begin() as conn:
            ...
            eq_(
                conn.scalar(select(boolean_table.c.id).where(~boolean_table.c.value)), 2
            )
            eq_(
                conn.scalar(
                    select(boolean_table.c.id).where(
...
class JSONTest(_LiteralRoundTripFixture, fixtures.TablesTest):
    ...
    def _index_fixtures(include_comparison):
        ...
        elements = [
            ...
            ("numeric", 99998969694839.983485848),
            ...
            ("_decimal", decimal.Decimal("99939.983485848")),
        ] + json_elements
    ...
    def _json_value_insert(self, connection, datatype, value, data_element):
        ...
        if datatype == "_decimal":
            ...
            connection.execute(
                data_table.insert(),
                {"name": "row1", "data": data_element, "nulldata": data_element},
            )
    ...
    @testing.combinations(
        ("parameters",), ("multiparameters",), ("values",), argnames="insert_type"
    )
    ...
    def test_unicode_round_trip(self):
        ...
        with config.db.begin() as conn:
            conn.execute(
                ...
                {
                    "name": "r1",
                    "data": {"réve🐍 illé": "réve🐍 illé", "data": {"k1": "drôl🐍e"}},
                },
            )

            eq_(
                conn.scalar(select(self.tables.data_table.c.data)),
                {"réve🐍 illé": "réve🐍 illé", "data": {"k1": "drôl🐍e"}},
            )
...
class JSONLegacyStringCastIndexTest(_LiteralRoundTripFixture, fixtures.TablesTest):
    ...
    def test_string_cast_crit_spaces_in_key(self):
        ...
        self._test_index_criteria(
            and_(
                name.in_(["r1", "r2", "r3"]), cast(col["key two"], String) == '"value2"'
            ),
            "r2",
        )
    ...
    def test_string_cast_crit_string_path(self):
        col = self.tables.data_table.c["data"]
        self._test_index_criteria(
            cast(col[("nested", "elem2", "elem3", "elem4")], String) == '"elem5"', "r5"
        )
...
class UuidTest(_LiteralRoundTripFixture, fixtures.TablesTest):
    ...
    def define_tables(cls, metadata):
        Table(
            ...
            Column("uuid_text_data_nonnative", Uuid(as_uuid=False, native_uuid=False)),
        )
    ...
    def test_uuid_round_trip(self, connection):
        ...
        row = connection.execute(
            select(uuid_table.c.uuid_data, uuid_table.c.uuid_data_nonnative).where(
                uuid_table.c.uuid_data == data, uuid_table.c.uuid_data_nonnative == data
            )
        ).first()
        eq_(row, (data, data))
    ...
    def test_uuid_text_round_trip(self, connection):
        ...
        connection.execute(
            uuid_table.insert(),
            {"id": 1, "uuid_text_data": data, "uuid_text_data_nonnative": data},
        )
        row = connection.execute(
            select(
                uuid_table.c.uuid_text_data, uuid_table.c.uuid_text_data_nonnative
            ).where(
    ...
    def test_literal_text(self, literal_round_trip):
        data = str(uuid.uuid4())
        literal_round_trip(
            self.datatype(as_uuid=False), [data], [data], filter_=lambda x: x.lower()
        )
