...
class HasTableTest(OneConnectionTablesTest):
    ...
    def define_views(cls, metadata):
        ...
        if testing.requires.schemas.enabled:
            ...
            event.listen(metadata, "after_create", DDL(query))
            event.listen(
                metadata, "before_drop", DDL("DROP VIEW %s.vv" % (config.test_schema))
            )
...
class HasIndexTest(fixtures.TablesTest):
    ...
    def test_has_index(self, kind, connection, metadata):
        ...
        tbl = Table(
            "test_table_2", metadata, Column("foo", Integer), Index("my_idx_3", "foo")
        )
...
class BizarroCharacterFKResolutionTest(fixtures.TestBase):
    ...
    @testing.combinations(
        ("plain",), ("(2)",), ("per % cent",), ("[brackets]",), argnames="tablename"
    )
...
def _multi_combination(fn):
    schema = testing.combinations(
        None, (lambda: config.test_schema, testing.requires.schemas), argnames="schema"
    )
    scope = testing.combinations(
        ObjectScope.DEFAULT, ObjectScope.TEMPORARY, ObjectScope.ANY, argnames="scope"
    )
...
class ComponentReflectionTest(ComparesTables, OneConnectionTablesTest):
    ...
    def define_reflected_tables(cls, metadata, schema):
        ...
        users = Table(
            ...
            sa.CheckConstraint(
                "test2 > 0", name="zz_test2_gt_zero", comment="users check constraint"
            ),
        ...
        Table(
            ...
            Column(
                "id_user", sa.Integer, ForeignKey("%susers.user_id" % schema_prefix)
            ),
            Column("data", sa.String(30), unique=True),
            sa.CheckConstraint(
                "address_id > 0 AND address_id < 1000", name="address_id_gt_zero"
            ),
            sa.UniqueConstraint(
                "address_id",
                "dingaling_id",
    ...
    def exp_options(
        self, schema=None, scope=ObjectScope.ANY, kind=ObjectKind.ANY, filter_names=None
    ):
    ...
    def exp_columns(
        ...
        tables = {
            ...
            (schema, "comment_test"): [
                ...
                col("d2", comment=r"""Comment types type speedily ' " \ '' Fun!"""),
            ...
            (schema, self.temp_table_name()): [pk("id"), col("name"), col("foo")],
        }
        res = self._resolve_kind(kind, tables, views, materialized)
        res = self._resolve_names(schema, scope, filter_names, res)
        return res
    ...
    def exp_fks(
        ...
        def fk(
            cols, ref_col, ref_table, ref_schema=schema, name=mock.ANY, comment=None
        ):
            return {
    ...
    def exp_indexes(
        ...
        def idx(
            *cols, name, unique=False, column_sorting=None, duplicates=False, fk=False
        ):
        ...
        tables = {
            ...
            (schema, "remote_table_2"): [],
            (schema, "noncol_idx_test_nopk"): [
                *idx("q", name="noncol_idx_nopk", column_sorting={"q": ("desc",)})
            ],
            (schema, "noncol_idx_test_pk"): [
    ...
    def exp_ccs(
        ...
        tables = {
            (schema, "users"): [
                ...
                cc("test2 > 0", "zz_test2_gt_zero", comment="users check constraint"),
            ],
            (schema, "dingalings"): [
                cc("address_id > 0 and address_id < 1000", name="address_id_gt_zero")
            ],
            (schema, "email_addresses"): [],
            (schema, "comment_test"): [],
            (schema, "no_constraints"): [],
    ...
    @testing.combinations(
        ...
        (True, True, testing.requires.schemas + testing.requires.view_reflection),
    ...
    def test_get_inter_schema_foreign_keys(self, connection):
        ...
        is_true(
            fkey2["referred_schema"] in (None, connection.dialect.default_schema_name)
        )
    ...
    def test_get_temp_table_indexes(self, connection):
        ...
        eq_([idx for idx in indexes if idx["name"] == "user_tmp_ix"], expected)
    ...
    def test_get_multi_columns(self, get_multi_exp, schema, scope, kind, use_filter):
        insp, kws, exp = get_multi_exp(
            schema, scope, kind, use_filter, Inspector.get_columns, self.exp_columns
        )
    ...
    def test_get_multi_pk_constraint(
        ...
        insp, kws, exp = get_multi_exp(
            schema, scope, kind, use_filter, Inspector.get_pk_constraint, self.exp_pks
        )
        for kw in kws:
            insp.clear_cache()
    ...
    def test_get_multi_foreign_keys(
        ...
        insp, kws, exp = get_multi_exp(
            schema, scope, kind, use_filter, Inspector.get_foreign_keys, self.exp_fks
        )
        for kw in kws:
            insp.clear_cache()
    ...
    def test_get_multi_indexes(self, get_multi_exp, schema, scope, kind, use_filter):
        insp, kws, exp = get_multi_exp(
            schema, scope, kind, use_filter, Inspector.get_indexes, self.exp_indexes
        )
        for kw in kws:
            insp.clear_cache()
    ...
    @testing.combinations(
        ...
        "get_columns",
        ("get_pk_constraint", testing.requires.primary_key_constraint_reflection),
        ("get_foreign_keys", testing.requires.foreign_key_constraint_reflection),
        ("get_indexes", testing.requires.index_reflection),
        ("get_unique_constraints", testing.requires.unique_constraint_reflection),
        ("get_check_constraints", testing.requires.check_constraint_reflection),
...
class TableNoColumnsTest(fixtures.TestBase):
    ...
    def view_no_columns(self, connection, metadata):
        Table("empty", metadata)
        event.listen(
            metadata, "after_create", DDL("CREATE VIEW empty_v AS SELECT * FROM empty")
        )
...
class ComponentReflectionTestExtra(ComparesIndexes, fixtures.TestBase):
    ...
    def ck_eq(self, reflected, expected):
        ...
        expected = sorted(expected, key=lambda item: (item["sqltext"]))
    ...
    def test_check_constraint_no_constraint(self, metadata, inspect_for_table):
        with inspect_for_table("no_constraints") as (schema, inspector):
            Table(
                "no_constraints", metadata, Column("data", sa.String(20)), schema=schema
            )

        self.ck_eq(inspector.get_check_constraints("no_constraints", schema=schema), [])
    ...
    def test_check_constraint_inline(
        ...
        self.ck_eq(
            reflected,
            [{"name": constraint_name or mock.ANY, "sqltext": "a > 1 and a < 5"}],
        )
    ...
    def test_get_foreign_key_options(
        ...
        Table(
            "x", metadata, Column("id", Integer, primary_key=True), test_needs_fk=True
        )
