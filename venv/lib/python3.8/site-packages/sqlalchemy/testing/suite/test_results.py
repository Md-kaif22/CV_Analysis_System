...
class RowFetchTest(fixtures.TablesTest):
    ...
    def insert_data(cls, connection):
        connection.execute(
            ...
            [{"id": 1, "data": "d1"}, {"id": 2, "data": "d2"}, {"id": 3, "data": "d3"}],
        )
    ...
    def test_row_with_dupe_names(self, connection):
        result = connection.execute(
            select(
                self.tables.plain_pk.c.data, self.tables.plain_pk.c.data.label("data")
...
class PercentSchemaNamesTest(fixtures.TablesTest):
    ...
    def define_tables(cls, metadata):
        ...
        cls.tables.lightweight_percent_table = sql.table(
            "percent%table", sql.column("percent%"), sql.column("spaces % more spaces")
        )
    ...
    def test_executemany_returning_roundtrip(self, connection):
        ...
        result = connection.execute(
            percent_table.insert().returning(
                percent_table.c["percent%"], percent_table.c["spaces % more spaces"]
            ),
            [
                {"percent%": 7, "spaces % more spaces": 11},
                {"percent%": 9, "spaces % more spaces": 10},
...
class ServerSideCursorsTest(fixtures.TestBase, testing.AssertsExecutionResults):
    ...
    @testing.combinations(
        ...
        ("global_text", True, lambda stringify: text(stringify("select 1")), True),
        ...
        ("stmt_option", False, select(1).execution_options(stream_results=True), True),
        (
        ...
        ("text_no_ss", False, lambda stringify: text(stringify("select 42")), False),
        (
            "text_ss_option",
            False,
    ...
    def test_roundtrip_fetchmany(self, metadata):
        ...
        with engine.begin() as connection:
            test_table.create(connection, checkfirst=True)
            connection.execute(
                test_table.insert(), [dict(data="data%d" % i) for i in range(1, 20)]
            )
