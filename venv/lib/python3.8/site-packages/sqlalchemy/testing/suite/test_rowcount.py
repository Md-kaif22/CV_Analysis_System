...
class RowCountTest(fixtures.TablesTest):
    ...
    def define_tables(cls, metadata):
        Table(
            "employees",
            metadata,
            Column("employee_id", Integer, autoincrement=False, primary_key=True),
    ...
    def test_non_rowcount_scenarios_no_raise(self, connection, statement, close_first):
        ...
        if statement.update:
            r = connection.execute(
                employees_table.update().where(department == "C"), {"department": "Z"}
            )
        elif statement.delete:
            r = connection.execute(
                employees_table.delete().where(department == "C"), {"department": "Z"}
            )
    ...
    def test_update_rowcount2(self, connection):
        ...
        r = connection.execute(
            employees_table.update().where(department == "C"), {"department": "C"}
        )
        eq_(r.rowcount, 3)
    ...
    def test_multi_update_rowcount(self, connection):
        ...
        r = connection.execute(
            stmt,
            [{"emp_name": "Bob"}, {"emp_name": "Cynthia"}, {"emp_name": "nonexistent"}],
        )
