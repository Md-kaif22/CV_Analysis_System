...
def _expect_warnings_sqla_only(
    exc_cls, messages, regex=True, search_msg=False, assert_=True
):
...
def int_within_variance(expected, received, variance):
    ...
    assert abs(received - expected) < deviance, (
        "Given int value %s is not within %d%% of expected value %s"
        % (received, variance * 100, expected)
    )
...
class AssertsCompiledSQL:
    def assert_compile(
        ...
        if check_param_order and getattr(c, "params", None):
            ...
            ordered = all(
                positions[i - 1] < positions[i] for i in range(1, len(positions))
            )
...
class ComparesTables:
    def assert_tables_equal(
        self, table, reflected_table, strict_types=False, strict_constraints=True
    ):
