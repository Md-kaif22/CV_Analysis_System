...
class ExcludeConstraint(ColumnCollectionConstraint):
    ...
    @elements._document_text_coercion(
        "where", ":class:`.ExcludeConstraint`", ":paramref:`.ExcludeConstraint.where`"
    )
    ...
    def _set_parent(self, table, **kw):
        ...
        self._render_exprs = [
            (expr if not isinstance(expr, str) else table.c[expr], name, operator)
            for expr, name, operator in (self._render_exprs)
        ]

    def _copy(self, target_table=None, **kw):
        elements = [
            (schema._copy_expression(expr, self.parent, target_table), operator)
