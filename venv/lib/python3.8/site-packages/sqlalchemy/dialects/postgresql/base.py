...
class PGDialect(default.DefaultDialect):
    ...
    construct_arguments = [
        ...
        (schema.CheckConstraint, {"not_valid": False}),
        (schema.ForeignKeyConstraint, {"not_valid": False}),
        (schema.UniqueConstraint, {"nulls_not_distinct": None}),
    ]
    ...
    def _split_multihost_from_url(
        ...
        Tuple[None, None], Tuple[Tuple[Optional[str], ...], Tuple[Optional[int], ...]]
    ]:
    ...
    def get_view_names(self, connection, schema=None, **kw):
        return self._get_relnames_for_relkinds(
            connection, schema, pg_catalog.RELKINDS_VIEW, scope=ObjectScope.DEFAULT
        )

    @reflection.cache
    def get_materialized_view_names(self, connection, schema=None, **kw):
        return self._get_relnames_for_relkinds(
            connection, schema, pg_catalog.RELKINDS_MAT_VIEW, scope=ObjectScope.DEFAULT
        )
                    ...
                    == sql.cast(
                        sql.cast(
                            pg_catalog.pg_get_serial_sequence(
                                sql.cast(
                                    sql.cast(
                                        pg_catalog.pg_attribute.c.attrelid, REGCLASS
                                    ),
                                    TEXT,
                                ),
        ...
        default = (
            select(
                pg_catalog.pg_get_expr(
                    pg_catalog.pg_attrdef.c.adbin, pg_catalog.pg_attrdef.c.adrelid
                )
            )
    ...
    def _foreing_key_query(self, schema, has_filter_names, scope, kind):
        ...
        query = (
            ...
            .outerjoin(
                pg_class_ref, pg_class_ref.c.oid == pg_catalog.pg_constraint.c.confrelid
            )
            .outerjoin(
                pg_namespace_ref, pg_class_ref.c.relnamespace == pg_namespace_ref.c.oid
            )
            .outerjoin(
                pg_catalog.pg_description,
                pg_catalog.pg_description.c.objoid == pg_catalog.pg_constraint.c.oid,
            )
            .order_by(pg_catalog.pg_class.c.relname, pg_catalog.pg_constraint.c.conname)
            .where(self._pg_class_relkind_condition(relkinds))
        )
    ...
    def _index_query(self):
        ...
        return (
            ...
            .join(
                pg_class_index, pg_catalog.pg_index.c.indexrelid == pg_class_index.c.oid
            )
            .join(pg_catalog.pg_am, pg_class_index.c.relam == pg_catalog.pg_am.c.oid)
            .outerjoin(
                cols_sq, pg_catalog.pg_index.c.indexrelid == cols_sq.c.indexrelid
            )
            .outerjoin(
                pg_catalog.pg_constraint,
                sql.and_(
    ...
    def _comment_query(self, schema, has_filter_names, scope, kind):
        ...
        query = (
            select(
                pg_catalog.pg_class.c.relname, pg_catalog.pg_description.c.description
            )
            .select_from(pg_catalog.pg_class)
            .outerjoin(
                pg_catalog.pg_description,
    ...
    def get_multi_table_comment(
        ...
        return (
            ((schema, table), {"text": comment} if comment is not None else default())
    ...
    def _domain_query(self, schema):
        ...
        query = (
            select(
                pg_catalog.pg_type.c.typname.label("name"),
                pg_catalog.format_type(
                    pg_catalog.pg_type.c.typbasetype, pg_catalog.pg_type.c.typtypmod
                ).label("attype"),
            ...
            .outerjoin(con_sq, pg_catalog.pg_type.c.oid == con_sq.c.contypid)
        ...
        for domain in result.mappings():
            ...
            if domain["connames"]:
                ...
                sorted_constraints = sorted(
                    zip(domain["connames"], domain["condefs"]), key=lambda t: t[0]
                )
