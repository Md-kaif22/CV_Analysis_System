        ...
        if self._select_lastrowid:
            if self.dialect.use_scope_identity:
                conn._cursor_execute(
                    self.cursor, "SELECT scope_identity() AS lastrowid", (), self
                )
            else:
                conn._cursor_execute(
...
class MSDialect(default.DefaultDialect):
    ...
    construct_arguments = [
        ...
        (
            sa_schema.Index,
            {"clustered": None, "include": None, "where": None, "columnstore": None},
        ),
        (sa_schema.Column, {"identity_start": None, "identity_increment": None}),
    ]
    ...
    def get_table_names(self, connection, dbname, owner, schema, **kw):
        ...
        s = (
            ...
            .where(
                sql.and_(
                    tables.c.table_schema == owner, tables.c.table_type == "BASE TABLE"
                )
            )
    ...
    def get_view_names(self, connection, dbname, owner, schema, **kw):
        ...
        s = (
            sql.select(tables.c.table_name)
            .where(
                sql.and_(tables.c.table_schema == owner, tables.c.table_type == "VIEW")
            )
            .order_by(tables.c.table_name)
        )
...
where
    ...
    def get_table_comment(self, connection, table_name, schema=None, **kw):
        ...
        if comment:
            ...
            return self._default_or_error(
                connection, table_name, None, ReflectionDefaults.table_comment, **kw
            )
    ...
    def get_columns(self, connection, tablename, dbname, owner, schema, **kw):
        ...
        if owner:
            whereclause = sql.and_(
                columns.c.table_name == tablename, columns.c.table_schema == owner
            )
    ...
    def get_pk_constraint(self, connection, tablename, dbname, owner, schema, **kw):
        ...
        if pkeys:
            ...
            return self._default_or_error(
                connection, tablename, owner, ReflectionDefaults.pk_constraint, **kw
            )
...
"""
        ...
        if fkeys:
            ...
            return self._default_or_error(
                connection, tablename, owner, ReflectionDefaults.foreign_keys, **kw
            )
