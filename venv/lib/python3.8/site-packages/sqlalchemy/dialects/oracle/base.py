    ...
    def pre_exec(self):
        if self.statement and "_oracle_dblink" in self.execution_options:
            self.statement = self.statement.replace(
                dictionary.DB_LINK_PLACEHOLDER, self.execution_options["_oracle_dblink"]
            )
...
class OracleDialect(default.DefaultDialect):
    ...
    def _has_table_query(self):
        ...
        tables = (
            select(dictionary.all_tables.c.table_name, dictionary.all_tables.c.owner)
            .union_all(
                select(
    ...
    def has_table(self, connection, table_name, schema=None, dblink=None, **kw):
        ...
        cursor = self._execute_reflection(
            connection, self._has_table_query, dblink, returns_long=False, params=params
        )
    ...
    def get_multi_table_options(
        self, connection, *, schema, filter_names, scope, kind, dblink=None, **kw
    ):
        """Supported kw arguments are: ``dblink`` to reflect via a db link;
        ``oracle_resolve_synonyms`` to resolve names to synonyms
        """
    ...
    def get_multi_pk_constraint(
        self, connection, *, scope, schema, filter_names, kind, dblink=None, **kw
    ):
        """Supported kw arguments are: ``dblink`` to reflect via a db link;
        ``oracle_resolve_synonyms`` to resolve names to synonyms
        """
    ...
    def get_view_definition(
        self, connection, view_name, schema=None, dblink=None, **kw
    ):
        """Supported kw arguments are: ``dblink`` to reflect via a db link;
        ``oracle_resolve_synonyms`` to resolve names to synonyms
        """
    ...
    def get_multi_check_constraints(
        ...
        **kw,
        ...
        return (
            (key, (check_constraints[key] if key in check_constraints else default()))
            for key in (
                (schema, self.normalize_name(obj_name)) for obj_name in all_objects
            )
        )
