...
class TypeEngine(Visitable, Generic[_T]):
    ...
    def with_variant(
        self, type_: _TypeEngineArgument[Any], *dialect_names: str
    ) -> Self:
    ...
    def _generic_type_affinity(self,) -> Type[TypeEngine[_T]]:
        ...
        for t in self.__class__.__mro__:
            if (
                t.__module__ in ("sqlalchemy.sql.sqltypes", "sqlalchemy.sql.type_api")
...
class Emulated(TypeEngineMixin):
    ...
    def adapt_to_emulated(
        self, impltype: Type[Union[TypeEngine[Any], TypeEngineMixin]], **kw: Any
    ) -> TypeEngine[Any]:
...
class NativeForEmulated(TypeEngineMixin):
    ...
    def adapt_native_to_emulated(
        cls, impl: Union[TypeEngine[Any], TypeEngineMixin], **kw: Any
    ) -> TypeEngine[Any]:
...
class TypeDecorator(SchemaEventTarget, ExternalType, TypeEngine[_T]):
    ...
    @staticmethod
    def _create_td_comparator_type(impl: TypeEngine[Any],) -> _ComparatorFactory[Any]:
    ...
    def comparator_factory(  # type: ignore  # mypy properties bug
        ...
        if (
            TypeDecorator.Comparator in self.impl.comparator_factory.__mro__
        ):  # type: ignore # noqa: E501
    ...
    def sort_key_function(
        self
    ) -> Optional[Callable[[Any], Any]]:  # type: ignore # noqa: E501
