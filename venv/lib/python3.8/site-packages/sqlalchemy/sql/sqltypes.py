...
class HasExpressionLookup(TypeEngineMixin):
    ...
    class Comparator(TypeEngine.Comparator[_CT]):
        ...
        def _adapt_expression(
            self, op: OperatorType, other_comparator: TypeEngine.Comparator[Any]
    ...
    class Comparator(TypeEngine.Comparator[_T]):
        ...
        def _adapt_expression(
            ...
            if op is operators.add and isinstance(
                other_comparator, (Concatenable.Comparator, NullType.Comparator)
            ):
...
class Integer(HasExpressionLookup, TypeEngine[int]):
    ...
    def _expression_adaptations(self):
        return {
            operators.add: {Date: Date, Integer: self.__class__, Numeric: Numeric},
            operators.mul: {
                Interval: Interval,
...
class Numeric(HasExpressionLookup, TypeEngine[_N]):
    ...
    def _expression_adaptations(self):
        return {
            ...
            operators.truediv: {Numeric: self.__class__, Integer: self.__class__},
...
class _Binary(TypeEngine[bytes]):
    ...
    @util.ro_memoized_property
    def _generic_type_affinity(self,) -> Type[TypeEngine[bytes]]:
        return LargeBinary
    ...
    def _set_table(self, column, table):
        ...
        if self.metadata is None:
            ...
            event.listen(
                ...
                "before_create",
                util.portable_instancemethod(
                    self._on_metadata_create, {"variant_mapping": variant_mapping}
                ),
            )
            event.listen(
                table.metadata,
                "after_drop",
                util.portable_instancemethod(
                    self._on_metadata_drop, {"variant_mapping": variant_mapping}
                ),
            )
        ...
        try:
            ...
            if not self.validate_strings and isinstance(elem, str):
                ...
                raise LookupError(
                    ...
                    % (elem, self.name, langhelpers.repr_tuple_names(self.enums))
                ) from err
    ...
    def _set_table(self, column, table):
        ...
        e = schema.CheckConstraint(
            ...
            _create_rule=util.portable_instancemethod(
                self._should_create_constraint, {"variant_mapping": variant_mapping}
            ),
            _type_bound=True,
        )
...
class Boolean(SchemaType, Emulated, TypeEngine[bool]):
    ...
    def copy(self, **kw):
        ...
        return self.adapt(
            cast("Type[TypeEngine[Any]]", self.__class__), _create_events=True
        )
...
class Interval(Emulated, _AbstractInterval, TypeDecorator[dt.timedelta]):
    ...
    def bind_processor(self, dialect: Dialect) -> _BindProcessorType[dt.timedelta]:
        ...
        if impl_processor:
            ...
            def process(value: Optional[dt.timedelta],) -> Any:
                if value is not None:
...
class ARRAY(SchemaEventTarget, Indexable, Concatenable, TypeEngine[Sequence[Any]]):
    ...
    * When used with the Python ``getitem`` accessor, the number of dimensions
    ...
    class Comparator(
        Indexable.Comparator[Sequence[Any]], Concatenable.Comparator[Sequence[Any]]
    ):
...
class TableValueType(HasCacheKey, TypeEngine[Any]):
    ...
    _traverse_internals = [("_elements", InternalTraversal.dp_clauseelement_list)]
...
class Uuid(Emulated, TypeEngine[_UUID_RETURN]):
    ...
    **Python uuid objects**, from the Python
    ...
    @overload
    def __init__(
        self: Uuid[_python_UUID], as_uuid: Literal[True] = ..., native_uuid: bool = ...
    ):
        ...

    @overload
    def __init__(
        self: Uuid[str], as_uuid: Literal[False] = ..., native_uuid: bool = ...
    ):
        ...
