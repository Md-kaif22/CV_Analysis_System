...
def _boolean_compare(
    ...
    **kwargs: Any,
    ...
    if reverse:
        return OperatorExpression._construct_for_op(
            obj, expr, op, type_=result_type, negate=negate_op, modifiers=kwargs
        )
    else:
        return OperatorExpression._construct_for_op(
            expr, obj, op, type_=result_type, negate=negate_op, modifiers=kwargs
        )
...
def _match_impl(
    ...
    return _boolean_compare(
        ...
        operators.match_op,
        coercions.expect(
            roles.BinaryElementRole, other, expr=expr, operator=operators.match_op
        ),
...
def _between_impl(
    expr: ColumnElement[Any], op: OperatorType, cleft: Any, cright: Any, **kw: Any
) -> ColumnElement[Any]:
    ...
    return BinaryExpression(
        ...
        ExpressionClauseList._construct_for_list(
            ...
            type_api.NULLTYPE,
            coercions.expect(
                roles.BinaryElementRole, cleft, expr=expr, operator=operators.and_
            ),
            coercions.expect(
                roles.BinaryElementRole, cright, expr=expr, operator=operators.and_
            ),
            group=False,
        ),
        op,
...
def _regexp_match_impl(
    ...
    **kw: Any,
    ...
    return BinaryExpression(
        expr,
        coercions.expect(
            roles.BinaryElementRole, pattern, expr=expr, operator=operators.comma_op
        ),
        op,
...
operator_lookup: Dict[
    ...
    "any_op": (_scalar, util.immutabledict({"fn": CollectionAggregate._create_any})),
    "all_op": (_scalar, util.immutabledict({"fn": CollectionAggregate._create_all})),
    ...
    "desc_op": (_scalar, util.immutabledict({"fn": UnaryExpression._create_desc})),
    "asc_op": (_scalar, util.immutabledict({"fn": UnaryExpression._create_asc})),
    "nulls_first_op": (
        _scalar,
    ...
    "in_op": (_in_impl, util.immutabledict({"negate_op": operators.not_in_op})),
    "not_in_op": (_in_impl, util.immutabledict({"negate_op": operators.in_op})),
    "is_": (_boolean_compare, util.immutabledict({"negate_op": operators.is_})),
    "is_not": (_boolean_compare, util.immutabledict({"negate_op": operators.is_not})),
