...
def join_condition(
    ...
    return Join._join_condition(
        a, b, a_subset=a_subset, consider_as_foreign_keys=consider_as_foreign_keys
    )
...
class _repr_params(_repr_base):
    ...
    def __repr__(self) -> str:
        ...
        if self.ismulti:
            ...
            if len(self.params) > self.batches:
                ...
                return " ".join(
                    (
                        self._repr_multi(multi_params[: self.batches - 2], typ)[0:-1],
    ...
    def _repr_param_dict(self, params: _CoreSingleExecuteParams) -> str:
        trunc = self.trunc
        (items_first_batch, items_second_batch, trunclen) = self._get_batches(
            params.items()
        )
...
@overload
def reduce_columns(
    columns: Iterable[ColumnElement[Any]], *clauses: Optional[ClauseElement], **kw: bool
...
@overload
def reduce_columns(
    columns: _SelectIterable, *clauses: Optional[ClauseElement], **kw: bool
        ...
        if newcol is None and col in self.equivalents and col not in _seen:
            for equiv in self.equivalents[col]:
                newcol = self._corresponding_column(
                    equiv, require_embedded=require_embedded, _seen=_seen.union([col])
                )
