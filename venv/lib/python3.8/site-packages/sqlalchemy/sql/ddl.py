...
class DropSchema(_DropBase):
    ...
    def __init__(self, name: str, cascade: bool = False, if_exists: bool = False):
...
class SchemaGenerator(InvokeCreateDDLBase):
    ...
    def _can_create_index(self, index):
        ...
        return not self.checkfirst or not self.dialect.has_index(
            self.connection, index.table.name, index.name, schema=effective_schema
        )
    ...
    def visit_metadata(self, metadata):
        ...
        with self.with_ddl_events(
            metadata, tables=event_collection, checkfirst=self.checkfirst
        ):
            for seq in seq_coll:
...
class SchemaDropper(InvokeDropDDLBase):
    ...
    def visit_table(
        self, table, drop_ok=False, _is_metadata_operation=False, _ignore_sequences=()
    ):
...
def sort_tables_and_constraints(
    ...
    try:
        candidate_sort = list(
            topological.sort(fixed_dependencies.union(mutable_dependencies), tables)
        )
