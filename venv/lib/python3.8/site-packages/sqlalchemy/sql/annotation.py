...
class SupportsAnnotations(ExternallyTraversible):
    ...
    @overload
    def _deannotate(
        self, values: Sequence[str] = ..., clone: bool = ...
    ) -> SupportsAnnotations:
        ...

    def _deannotate(
        self, values: Optional[Sequence[str]] = None, clone: bool = False
    ) -> SupportsAnnotations:
        raise NotImplementedError()
...
class Annotated(SupportsAnnotations):
    ...
    def _deannotate(
        self, values: Optional[Sequence[str]] = None, clone: bool = True
    ) -> SupportsAnnotations:
        if values is None:
    ...
    def entity_namespace(self) -> _EntityNamespace:
        if "entity_namespace" in self._annotations:
            return cast(
                SupportsWrappingAnnotations, self._annotations["entity_namespace"]
            ).entity_namespace
        else:
...
def _new_annotation_type(
    ...
    annotated_classes[cls] = anno_cls = cast(
        Type[Annotated], type("Annotated%s" % cls.__name__, (base_cls, cls), {})
    )
...
def _prepare_annotations(
    target_hierarchy: Type[SupportsWrappingAnnotations], base_cls: Type[Annotated]
) -> None:
