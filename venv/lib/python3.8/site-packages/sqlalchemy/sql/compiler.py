...
class _ResultMapAppender(Protocol):
    def __call__(
        self, keyname: str, name: str, objects: Sequence[Any], type_: TypeEngine[Any]
    ) -> None:
        ...
        ...
        if the_rest:
            ...
            if froms:
                ...
                message = template.format(
                    stmt_type=stmt_type, froms=froms_str, start=self.froms[start_with]
                )
...
class SQLCompiler(Compiled):
    ...
    _cache_key_bind_match: Optional[
        Tuple[
            Dict[BindParameter[Any], List[BindParameter[Any]]],
            Dict[str, BindParameter[Any]],
        ]
    ] = None
        ...
        if self.state is CompilerState.STRING_APPLIED:
            ...
            if self._render_postcompile:
                parameters = self.construct_params(
                    escape_names=False, _no_postcompile=True
                )
    ...
    def _nested_result(self):
        ...
        result_columns, ordered_columns = (self._result_columns, self._ordered_columns)
    ...
    def construct_expanded_state(
        ...
        """
        parameters = self.construct_params(
            params, escape_names=escape_names, _no_postcompile=True
        )
    ...
    def _process_parameters_for_postcompile(
        self, parameters: _MutableCoreSingleExecuteParams, _populate_self: bool = False
    ) -> ExpandedState:
        ...
        * literal_binds compiled with the literal_execute flag.  Used for
            ...
            if parameter in self.literal_execute_params:
                if escaped_name not in replacement_expressions:
                    replacement_expressions[
                        ...
                        parameter, render_literal_value=parameters.pop(escaped_name)
                    )
                continue
                ...
                if not parameter.literal_execute:
                    ...
                    if parameter.type._is_tuple_type:
                        ...
                        new_processors.update(
                            ("%s_%s_%s" % (name, i, j), tuple_processors[name][j - 1])
        ...
        expanded_state = ExpandedState(
            statement, parameters, new_processors, new_positiontup, expanded_parameters
        )
    ...
    def visit_override_binds(self, override_binds, **kw):
        ...
        for k in override_binds.translate:
            ...
            new_bp = bp._with_value(
                override_binds.translate[bp.key], maintain_key=True, required=False
            )
        ...
        if self.dialect._bind_typing_render_casts and typ_dialect_impl.render_bind_cast:
            ...
            def _render_bindtemplate(name):
                return self.render_bind_cast(
                    parameter.type, typ_dialect_impl, bind_template % {"name": name}
                )
        ...
        **kw,
        ...
        if bindparam.expanding:
            ...
            to_update, replacement_expr = leep(
                bindparam, value, bind_expression_template=bind_expression_template
            )
            ...
            if not cte_pre_alias_name and cte not in self_ctes:
                ...
                if cte.recursive:
                    ...
                    recur_cols = [
                        ...
                        for (_, proxy_name, fallback_label_name, c, repeated) in (
                            col_source._generate_columns_plus_names(True)
                        )
                        if not repeated
                    ]
    ...
    def _add_to_result_map(
        self, keyname: str, name: str, objects: Tuple[Any, ...], type_: TypeEngine[Any]
    ) -> None:
    ...
    def _deliver_insertmanyvalues_batches(
        ...
        while batches:
            ...
            if self.positional:
                ...
                replaced_statement = statement.replace(
                    "__EXECMANY_TOKEN__", ", ".join(replaced_values_clauses)
                )
    ...
    def visit_insert(
        ...
        if returning_cols:
            ...
            returning_clause = self.returning_clause(
                insert_stmt, returning_cols, populate_result_map=toplevel
            )
    ...
    def visit_delete(self, delete_stmt, visiting_cte=None, **kw):
        ...
        try:
            table_text = self.delete_table_clause(
                delete_stmt, delete_stmt.table, extra_froms, from_linter=from_linter
            )
        except TypeError:
