...
class AssociationProxyInstance(SQLORMOperations[_T]):
    ...
    @classmethod
    def for_proxy(
        cls, parent: AssociationProxy[_T], owning_class: Type[Any], parent_instance: Any
    ) -> AssociationProxyInstance[_T]:
    ...
    def local_attr(self) -> SQLORMOperations[Any]:
        ...
        """
        return cast(
            "SQLORMOperations[Any]", getattr(self.owning_class, self.target_collection)
        )
    ...
    def _new(
        ...
        if collection_class is list:
            return (
                collection_class,
                cast(
                    _T, _AssociationList(lazy_collection, creator, getter, setter, self)
                ),
            )
            ...
            return (
                collection_class,
                cast(
                    _T, _AssociationDict(lazy_collection, creator, getter, setter, self)
                ),
            )
            ...
            return (
                collection_class,
                cast(
                    _T, _AssociationSet(lazy_collection, creator, getter, setter, self)
                ),
            )
        else:
            raise exc.ArgumentError(
    ...
    def _criterion_exists(
        self, criterion: Optional[_ColumnExpressionArgument[bool]] = None, **kwargs: Any
    ) -> ColumnElement[bool]:
...
class ObjectAssociationProxyInstance(AssociationProxyInstance[_T]):
    ...
    def __eq__(
        ...
        if obj is None:
            return or_(
                self._comparator.has(**{self.value_attr: obj}), self._comparator == None
            )
        else:
...
class _AssociationDict(_AssociationCollection[_VT], MutableMapping[_KT, _VT]):
    ...
    def _bulk_replace(
        self, assoc_proxy: AssociationProxyInstance[Any], values: Mapping[_KT, _VT]
    ) -> None:
        existing = set(self)
...
class _AssociationSet(_AssociationSingleItem[_T], MutableSet[_T]):
    ...
    def __ixor__(
        self, other: AbstractSet[_S]
    ) -> MutableSet[Union[_T, _S]]:  # type: ignore  # noqa: E501
        if not collections._set_binops_check_strict(self, other):
            raise NotImplementedError()
