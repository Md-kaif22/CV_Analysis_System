...
_NonAmbigCursorKeyMapRecType = Tuple[
    int, int, List[Any], str, str, Optional["_ResultProcessorType[Any]"], str
]
...
class CursorResultMetaData(ResultMetaData):
    ...
    def _for_freeze(self) -> ResultMetaData:
        return SimpleResultMetaData(
            self._keys, extra=[self._keymap[key][MD_OBJECTS] for key in self._keys]
        )
    ...
    def __init__(
        self, parent: CursorResult[Any], cursor_description: _DBAPICursorDescription
    ):
    ...
    def _merge_textual_cols_by_position(
        ...
        for (idx, colname, untranslated, coltype) in self._colnames_from_description(
    ...
    def _merge_cols_by_name(
        self, context, cursor_description, result_columns, loose_column_name_matching
    ):
        ...
        for ridx, elem in enumerate(result_columns):
            ...
            if loose_column_name_matching:
                ...
                for r_key in elem[RM_OBJECTS]:
                    d.setdefault(
                        r_key, (elem[RM_NAME], elem[RM_OBJECTS], elem[RM_TYPE], ridx)
                    )
        return d
...
class ResultFetchStrategy:
    ...
    def yield_per(
        self, result: CursorResult[Any], dbapi_cursor: Optional[DBAPICursor], num: int
    ) -> None:
        return
...
class BufferedRowCursorFetchStrategy(CursorFetchStrategy):
    ...
    def __init__(
        self, dbapi_cursor, execution_options, growth_factor=5, initial_buffer=None
    ):
...
class CursorResult(Result[_T]):
    ...
    def __init__(
        ...
        if cursor_description is not None:
            ...
            _make_row = functools.partial(
                Row, metadata, metadata._effective_processors, metadata._key_to_index
            )
    ...
    def splice_horizontally(self, other):
        ...
        total_rows = [
            tuple(r1) + tuple(r2)
            for r1, r2 in zip(
                list(self._raw_row_iterator()), list(other._raw_row_iterator())
            )
        ]
        ...
        clone.cursor_strategy = FullyBufferedCursorFetchStrategy(
            None, initial_buffer=total_rows
        )
