...
class Connection(ConnectionEventsTarget, inspection.Inspectable["Inspector"]):
    ...
    def _execute_ddl(
        ...
        if self._has_events or self.engine._has_events:
            self.dispatch.after_execute(
                self, ddl, event_multiparams, event_params, exec_opts, ret
            )
        return ret
    ...
    def _invoke_before_exec_event(
        ...
        Any, _CoreMultiExecuteParams, _CoreMultiExecuteParams, _CoreSingleExecuteParams
    ]:
            ...
            if not self.engine.hide_parameters:
                self._log_info(
                    ...
                    sql_util._repr_params(
                        effective_parameters, batches=10, ismulti=context.executemany
                    ),
                )
            else:
                self._log_info(
                    "[%s] [SQL parameters hidden due to hide_parameters=True]", stats
                )
    ...
    def _exec_insertmany_context(
        self, dialect: Dialect, context: ExecutionContext
    ) -> CursorResult[Any]:
        ...
        for imv_batch in dialect._deliver_insertmanyvalues_batches(
            ...
            if imv_batch.processed_setinputsizes:
                try:
                    dialect.do_set_input_sizes(
                        context.cursor, imv_batch.processed_setinputsizes, context
                    )
                except BaseException as e:
                    self._handle_dbapi_exception(
                        e,
            ...
            if engine_events:
                for fn in self.dispatch.before_cursor_execute:
                    sub_stmt, sub_params = fn(
                        self, cursor, sub_stmt, sub_params, context, True
                    )
            ...
            if self._echo:
                ...
                if not self.engine.hide_parameters:
                    self._log_info(
                        "[%s] %r",
                        stats,
                        sql_util._repr_params(sub_params, batches=10, ismulti=False),
                    )
                else:
                    self._log_info(
            ...
            try:
                for fn in do_execute_dispatch:
                    if fn(cursor, sub_stmt, sub_params, context):
                        break
                else:
        ...
        if not self._is_disconnect:
            self._is_disconnect = (
                ...
                and self.dialect.is_disconnect(
                    e, self._dbapi_connection if not self.invalidated else None, cursor
                )
