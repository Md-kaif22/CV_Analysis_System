...
class Inspector(inspection.Inspectable["Inspector"]):
    ...
    def has_index(
        self, table_name: str, index_name: str, schema: Optional[str] = None, **kw: Any
    ) -> bool:
        ...
        with self._operation_context() as conn:
            return self.dialect.has_index(
                conn, table_name, index_name, schema, info_cache=self.info_cache, **kw
            )
    ...
    def sort_tables_on_foreign_key_dependency(
        self, consider_schemas: Collection[Optional[str]] = (None,), **kw: Any
    ) -> List[
        Tuple[
            ...
            for (_, tname), fkeys in schema_fkeys.items():
                ...
                for fkey in fkeys:
                    if (
                        ...
                        tuples.add(
                            (
                                (fkey["referred_schema"], fkey["referred_table"]),
                                (schname, tname),
                            )
                        )
        try:
    ...
    def reflect_table(
        ...
        for col_d in _reflect_info.columns[table_key]:
            ...
            self._reflect_column(
                table, col_d, include_columns, exclude_columns, cols_by_orig_name
            )
    ...
    def _reflect_indexes(
        ...
        for index_d in indexes:
            ...
            for index, c in enumerate(columns):
                ...
                sa_schema.Index(
                    name, *idx_elements, _table=table, unique=unique, **dialect_options
                )
        ...
        for const_d in constraints:
            ...
            table.append_constraint(
                sa_schema.UniqueConstraint(
                    *constrained_cols, name=conname, comment=comment, **dialect_options
                )
            )
