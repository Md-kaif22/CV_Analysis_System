...
(CACHE_HIT, CACHE_MISS, CACHING_DISABLED, NO_CACHE_KEY, NO_DIALECT_SUPPORT) = list(
    CacheStats
)
...
class DefaultDialect(Dialect):
    ...
    def _deliver_insertmanyvalues_batches(
        self, connection, cursor, statement, parameters, generic_setinputsizes, context
    ):
                        ...
                        if composite_sentinel:
                            _composite_sentinel_proc = [
                                ...
                                for col, cursor_desc in zip(
                                    imv.sentinel_columns, cursor.description[-_nsc:]
                                )
                            ]
                        else:
    ...
    def _default_multi_reflect(
        self, single_tbl_method, connection, kind, schema, filter_names, scope, **kw
    ):
        names_fns = []
        temp_names_fns = []
        ...
        if not parameters:
            self.compiled_parameters = [
                compiled.construct_params(
                    extracted_parameters=extracted_parameters, escape_names=False
                )
            ]
        else:
            self.compiled_parameters = [
