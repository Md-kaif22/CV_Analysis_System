...
class ORMDMLState(AbstractORMCompileState):
            ...
            if isinstance(k, str):
                ...
                if desc is NO_VALUE:
                    ...
                    yield from core_get_crud_kv_pairs(
                        statement, desc._bulk_update_tuples(v), needs_to_be_cacheable
                    )
                ...
                yield from core_get_crud_kv_pairs(
                    statement, attr._bulk_update_tuples(v), needs_to_be_cacheable
                )
            else:
                yield (
                    k,
    ...
    def _get_crud_kv_pairs(cls, statement, kv_iterator, needs_to_be_cacheable):
        ...
        return list(
            cls._get_orm_crud_kv_pairs(
                plugin_subject.mapper, statement, kv_iterator, needs_to_be_cacheable
            )
        )
    ...
    def _get_matched_objects_on_criteria(cls, update_options, states):
        ...
        for obj, state, dict_ in raw_data:
            ...
            if (
                ...
                result.append(
                    (obj, state, dict_, evaled_condition is evaluator._EXPIRED_OBJECT)
                )
        return result
...
class BulkORMInsert(ORMDMLState, InsertDMLState):
    ...
    def create_for_statement(cls, statement, compiler, **kw) -> BulkORMInsert:
        self = cast(
            BulkORMInsert, super().create_for_statement(statement, compiler, **kw)
        )
...
class BulkORMUpdate(BulkUDCompileState, UpdateDMLState):
    ...
    def _do_post_synchronize_bulk_evaluate(
        ...
        for param in params:
            identity_key = mapper.identity_key_from_primary_key(
                (param[key] for key in pk_keys), update_options._identity_token
            )
    ...
    def _do_post_synchronize_evaluate(cls, session, statement, result, update_options):
        matched_objects = cls._get_matched_objects_on_criteria(
            update_options, session.identity_map.all_states()
        )
    ...
    def _do_post_synchronize_fetch(cls, session, statement, result, update_options):
        ...
        cls._apply_update_set_values_to_objects(
            ...
            result.context.compiled_parameters[0],
            [
                (obj, attributes.instance_state(obj), attributes.instance_dict(obj))
                for obj in objs
            ],
            result.prefetch_cols(),
            result.postfetch_cols(),
