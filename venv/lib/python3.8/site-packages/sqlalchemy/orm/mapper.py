...
class Mapper(
    ...
    def __init__(
        ...
        self._sort_key = "%s.%s" % (self.class_.__module__, self.class_.__name__)
        ...
        self.polymorphic_on = (
            coercions.expect(  # type: ignore
                roles.ColumnArgumentOrKeyRole, polymorphic_on, argname="polymorphic_on"
            )
            ...
            if self.single:
                ...
                if self.concrete:
                    ...
                    self._inherits_equated_pairs = sql_util.criterion_as_pairs(
                        self.persist_selectable.onclause, consider_as_foreign_keys=fks
                    )
            else:
                self.persist_selectable = self.local_table
    ...
    def _configure_pks(self) -> None:
        ...
        for fc in set(self.tables).union([self.persist_selectable]):
            if fc.primary_key and pk_cols.issuperset(fc.primary_key):
                ...
                self._pks_by_table[
                    fc
                ] = util.ordered_column_set(  # type: ignore  # noqa: E501
                    fc.primary_key
                ).intersection(
                    pk_cols
                )
            self._cols_by_table[fc] = util.ordered_column_set(
                fc.c
            ).intersection(  # type: ignore  # noqa: E501
                all_cols
            )
        ...
        if self._primary_key_argument:
            coerced_pk_arg = [
                ...
                for c in (
                    coercions.expect(
                        roles.DDLConstraintColumnRole, coerce_pk, argname="primary_key"
                    )
        ...
        if setter:
            ...
            def _set_polymorphic_identity(state):
                ...
                state.get_impl(polymorphic_key).set(
                    state, dict_, polymorphic_identity, None
                )
    ...
    def _configure_property(
        ...
        *,
        ...
        # get replaced.
            ...
            and not isinstance(prop, descriptor_props.SynonymProperty)
            and not isinstance(
                self._props.get(key, None), descriptor_props.ConcreteInheritedProperty
            )
        ):
            util.warn_deprecated(
                f"User-placed attribute {self.class_.__name__}.{key} on "
    ...
    def _reconcile_prop_with_incoming_columns(
        ...
        if (
            ...
            msg = (
                ...
                "explicitly." % (existing_prop.columns[-1], incoming_column, key)
            )
            if warn_only:
                util.warn(msg)
            else:
        ...
        self._log(
            "inserting column to existing list " "in properties.ColumnProperty %s", key
        )
        return new_prop  # type: ignore

    @util.preload_module("sqlalchemy.orm.descriptor_props")
    def _property_from_column(
        self, key: str, column: KeyedColumnElement[Any]
    ) -> ColumnProperty[Any]:
        ...
        if isinstance(prop, properties.ColumnProperty):
            return self._reconcile_prop_with_incoming_columns(
                key, prop, single_column=column, warn_only=prop.parent is not self
            )
    ...
    def _has_aliased_polymorphic_fromclause(self):
        ...
        return self.with_polymorphic and isinstance(
            self.with_polymorphic[1], expression.AliasedReturnsRows
        )
    ...
    @HasMemoized.memoized_attribute
    def _server_default_cols(self,) -> Mapping[FromClause, FrozenSet[Column[Any]]]:
        return {
            table: frozenset(
                [
                    col
    ...
    @HasMemoized.memoized_attribute
    def _server_onupdate_default_col_keys(self,) -> Mapping[FromClause, FrozenSet[str]]:
        return {
            table: frozenset(col.key for col in cols if col.key is not None)
    ...
    def __clause_element__(self):
        ...
        if self.persist_selectable is not self.local_table:
            ...
            annotations["dml_table"] = self.local_table._annotate(
                {"entity_namespace": self, "parententity": self, "parentmapper": self}
    ...
    def _get_clause(self):
        ...
        """
        params = [
            (primary_key, sql.bindparam("pk_%d" % idx, type_=primary_key.type))
    ...
    def _is_userland_descriptor(self, assigned_name: str, obj: Any) -> bool:
        if isinstance(
            obj,
            (_MappedAttribute, instrumentation.ClassManager, expression.ColumnElement),
        ):
            return False
        else:
    ...
    def identity_key_from_primary_key(
        self, primary_key: Tuple[Any, ...], identity_token: Optional[Any] = None
    ) -> _IdentityKeyType[_O]:
        """Return an identity-map key for use in storing/retrieving an
            ...
            if item_type is prp:
                ...
                queue = deque(
                    prop.cascade_iterator(
                        type_, parent_state, parent_dict, visited_states, halt_on
                    )
                )
                if queue:
    ...
    def _sorted_tables(self):
        ...
        sorted_ = sql_util.sort_tables(
            table_to_mapper, skip_fn=skip, extra_dependencies=extra_dependencies
        )
    ...
    def _table_to_equated(self):
        ...
        result: util.defaultdict[
            Table,
            List[
                Tuple[Mapper[Any], List[Tuple[ColumnElement[Any], ColumnElement[Any]]]]
            ],
...
def _configure_registries(registries: Set[_RegistryType], cascade: bool) -> None:
    ...
    with _CONFIGURE_MUTEX:
        ...
        try:
            ...
            Mapper.dispatch._for_class(
                Mapper
            ).before_configured()  # type: ignore # noqa: E501
