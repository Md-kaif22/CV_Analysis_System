...
class UninstrumentedColumnLoader(LoaderStrategy):
    ...
    def create_row_processor(
        self, context, query_entity, path, loadopt, mapper, result, adapter, populators
    ):
        pass
    ...
    def _memoized_attr__simple_lazy_clause(self):
        ...
        def visit_bindparam(bindparam):
            if bindparam._identifying_key in bind_to_col:
                params.append(
                    (bindparam.key, bind_to_col[bindparam._identifying_key], None)
                )
...
class LoadLazyAttribute:
    ...
    def __call__(self, state, passive=attributes.PASSIVE_OFF):
        ...
        return strategy._load_for_state(
            state, passive, loadopt=self.loadopt, extra_criteria=self.extra_criteria
        )
...
class SubqueryLoader(PostLoader):
    ...
    def _get_leftmost(
        self, orig_query_entity_index, subq_path, current_compile_state, is_root
    ):
        ...
        leftmost_attr = [
            getattr(
                new_subq_path.path[0].entity, leftmost_mapper._columntoproperty[c].key
            )
        ...
        if loadopt and loadopt._extra_criteria:
            new_options += (
                orm_util.LoaderCriteriaOption(
                    self.entity, loadopt._generate_extra_criteria(context)
                ),
            )
    ...
    def _setup_query_from_rowproc(
        self, context, query_entity, path, entity, loadopt, adapter
    ):
        ...
        (
            ...
            orig_query_entity_index, rewritten_path, orig_compile_state, is_root
        )
        ...
        q = self._setup_options(
            context, q, subq_path, rewritten_path, orig_query, effective_entity, loadopt
        )
    ...
    def create_row_processor(
        ...
        subq = self._setup_query_from_rowproc(
            context, query_entity, path, path[-1], loadopt, adapter
        )
        ...
        **kwargs,
        ...
        if user_defined_adapter is not False:
            ...
            (
                ...
                compile_state, query_entity, path, adapter, user_defined_adapter
            )
...
class SelectInLoader(PostLoader, util.MemoizedSlots):
    ...
    def _load_for_path(
            ...
            for state, overwrite in states:
                ...
                related_ident = tuple(
                    mapper._get_state_attr_by_column(
                        state, state_dict, lk, passive=attributes.PASSIVE_NO_FETCH
                    )
        ...
        if loadopt and loadopt._extra_criteria:
            new_options += (
                orm_util.LoaderCriteriaOption(
                    effective_entity, loadopt._generate_extra_criteria(context)
                ),
            )
        ...
        if query_info.load_only_child:
            self._load_via_child(
                our_states, none_states, query_info, q, context, execution_options
            )
        else:
            self._load_via_parent(our_states, query_info, q, context, execution_options)

    def _load_via_child(
        self, our_states, none_states, query_info, q, context, execution_options
    ):
            ...
            for key in chunk:
                ...
                for state, dict_, overwrite in our_states[key]:
                    ...
                    state.get_impl(self.key).set_committed_value(
                        state, dict_, related_obj if not uselist else [related_obj]
                    )
