...
class QueryContext:
    ...
    def __init__(
        ...
        user_passed_query: Union[Select[Any], FromStatement[Any]],
        params: _CoreSingleExecuteParams,
        session: Session,
        load_options: Union[
            Type[QueryContext.default_load_options], QueryContext.default_load_options
        ],
...
class AbstractORMCompileState(CompileState):
    ...
    def orm_pre_session_exec(
        cls, session, statement, params, execution_options, bind_arguments, is_pre_event
    ):
        raise NotImplementedError()

    @classmethod
    def orm_execute_statement(
        cls, session, statement, params, execution_options, bind_arguments, conn
    ) -> Result:
        result = conn.execute(
            statement, params or {}, execution_options=execution_options
        )
        return cls.orm_setup_cursor_result(
            session, statement, params, execution_options, bind_arguments, result
        )
...
class ORMSelectCompileState(ORMCompileState, SelectState):
        ...
        if select_statement._memoized_select_entities:
            self._memoized_entities = {
                memoized_entities: _QueryEntity.to_compile_state(
                    self, memoized_entities._raw_columns, [], is_current_entities=False
                )
    ...
    def from_statement(cls, statement, from_statement):
        from_statement = coercions.expect(
            roles.ReturnsRowsRole, from_statement, apply_propagate_attrs=statement
        )
    ...
    def _select_statement(
        ...
        statement = Select._create_raw_select(
            _raw_columns=raw_columns, _from_obj=from_obj, _label_style=label_style
        )
    ...
    def _join(self, args, entities_collection):
        for right, onclause, from_, flags in args:
                ...
                if from_ is not None:
                    if (
                        ...
                        raise sa_exc.InvalidRequestError(
                            ...
                            % (from_._annotations.get("parententity", from_), onclause)
                        )
            ...
            self._join_left_to_right(
                entities_collection, left, right, onclause, prop, isouter, full
            )
...
class _QueryEntity:
    ...
    def setup_dml_returning_compile_state(
        self, compile_state: ORMCompileState, adapter: DMLReturningColFilter
    ) -> None:
        raise NotImplementedError()
