    ...
    def update_stmt(existing_stmt=None):
        ...
        if needs_version_id:
            clauses._append_inplace(
                mapper.version_id_col
                == sql.bindparam(
                    mapper.version_id_col._label, type_=mapper.version_id_col.type
                )
            )
    ...
    for (
        ...
        if use_orm_update_stmt is not None:
            statement = statement._annotate(
                {"_emit_update_table": table, "_emit_update_mapper": mapper}
            )
...
def _emit_insert_statements(
    ...
    *,
    ...
    for ((connection, _, hasvalue, has_all_pks, has_all_defaults), records) in groupby(
        insert,
        lambda rec: (
            rec[4],  # connection
        ...
        if use_orm_insert_stmt is not None:
            statement = statement._annotate(
                {"_emit_insert_table": table, "_emit_insert_mapper": mapper}
            )
        ...
        if (
            ...
            if mapper.version_id_col is not None:
                statement = statement.return_defaults(
                    mapper.version_id_col, sort_by_parameter_order=bookkeeping
                )
            elif do_executemany:
                statement = statement.return_defaults(
...
def _emit_post_update_statements(base_mapper, uowtransaction, mapper, table, update):
    ...
    for key, records in groupby(
        update, lambda rec: (rec[3], set(rec[4]))  # connection  # parameter keys
    ):
        rows = 0
...
def _postfetch(
    ...
    for m, equated_pairs in mapper._table_to_equated[table]:
        sync.populate(
            state, m, state, m, equated_pairs, uowtransaction, mapper.passive_updates
        )
