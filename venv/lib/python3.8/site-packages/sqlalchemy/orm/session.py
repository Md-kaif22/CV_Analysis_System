...
JoinTransactionMode = Literal[
    "conditional_savepoint", "rollback_only", "control_fully", "create_savepoint"
]
    ...
    def identity_key(
        ...
        *,
        ...
        return util.preloaded.orm_util.identity_key(
            class_, ident, instance=instance, row=row, identity_token=identity_token
        )
        ...
        if params:
            if self.is_executemany:
                ...
                for _existing_params, _new_params in itertools.zip_longest(
                    exec_many_parameters, cast("List[Dict[str, Any]]", params)
                ):
...
class Session(_SessionClassMethods, EventTarget):
    ...
    def connection(
        ...
        return self._connection_for_bind(bind, execution_options=execution_options)
    ...
    def _execute_internal(
        ...
        *,
        ...
        if events_todo:
            if compile_state_cls is not None:
                ...
                (statement, execution_options) = compile_state_cls.orm_pre_session_exec(
                    self, statement, params, execution_options, bind_arguments, True
                )
        ...
        if compile_state_cls is not None:
            ...
            (statement, execution_options) = compile_state_cls.orm_pre_session_exec(
                self, statement, params, execution_options, bind_arguments, False
            )
        ...
        if compile_state_cls:
            result: Result[Any] = compile_state_cls.orm_execute_statement(
                self, statement, params or {}, execution_options, bind_arguments, conn
            )
        else:
            result = conn.execute(
                statement, params or {}, execution_options=execution_options
    ...
    def _expire_state(
        self, state: InstanceState[Any], attribute_names: Optional[Iterable[str]]
    ) -> None:
        ...
        *,
        ...
        if merged is None:
            if key_is_persistent and key in _resolve_conflict_map:
                ...
                merged = self.get(
                    mapper.class_, key[1], identity_token=key[2], options=options
                )
    ...
    def bulk_save_objects(
        ...
        if not preserve_order:
            ...
            obj_states = sorted(
                obj_states, key=lambda state: (id(state.mapper), state.key is not None)
            )

        def grouping_key(state: InstanceState[_O],) -> Tuple[Mapper[_O], bool]:
