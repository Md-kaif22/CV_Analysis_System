...
class _MapperConfig:
    ...
    properties: util.OrderedDict[
        str, Union[Sequence[NamedColumn[Any]], NamedColumn[Any], MapperProperty[Any]]
    ]
    ...
    def __init__(
        self, registry: _RegistryType, cls_: Type[Any], mapper_kw: _MapperKwArgs
    ):
...
class _ClassScanMapperConfig(_MapperConfig):
    ...
    column_copies: Dict[
        Union[MappedColumn[Any], Column[Any]], Union[MappedColumn[Any], Column[Any]]
    ]
    ...
    def _cls_attr_resolver(
        ...
        if not sa_dataclass_metadata_key:
            ...
            def local_attributes_for_class() -> (Iterable[Tuple[str, Any, Any, bool]]):
                return (
                    (name, cls_vars.get(name), cls_annotations.get(name), False)
                    for name in names
                )
                ...
                if name in _include_dunders:
                    ...
                    if isinstance(obj, (Column, MappedColumn)):
                        ...
                        self._collect_annotation(
                            name, obj._collect_return_annotation(), base, True, obj
                        )
    ...
    def _setup_dataclasses_transforms(self) -> None:
        ...
        field_list = [
            ...
            for key, anno, mapped_container in (
                (key, mapped_anno if mapped_anno else raw_anno, mapped_container)
                for key, (
                    raw_anno,
                    mapped_container,
                    mapped_anno,
        ...
        if self.table_fn is None and "__table__" not in clsdict_view and table is None:
            ...
            if tablename is not None:
                ...
                sorted_columns = sorted(
                    declared_columns, key=lambda c: column_ordering.get(c, 0)
                )
    ...
    def map(self, mapper_kw: _MapperKwArgs = util.EMPTY_DICT) -> Mapper[Any]:
        ...
        return self.set_cls_attribute(
            "__mapper__", mapper_cls(self.cls, self.local_table, **self.mapper_args)
        )
