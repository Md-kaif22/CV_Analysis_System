...
class _AbstractLoad(traversals.GenerativeOnTraversal, LoaderOption):
    ...
    def selectinload(
        self, attr: _AttrType, recursion_depth: Optional[int] = None
    ) -> Self:
        """Indicate that the given attribute should be loaded using
        ...
        return self._set_relationship_strategy(
            attr, {"lazy": "selectin"}, opts={"recursion_depth": recursion_depth}
        )
    ...
    def immediateload(
        ...
        """
        loader = self._set_relationship_strategy(
            attr, {"lazy": "immediate"}, opts={"recursion_depth": recursion_depth}
        )
        return loader
    ...
    def with_expression(
        self, key: _AttrType, expression: _ColumnExpressionArgument[Any]
    ) -> Self:
    ...
    def process_compile_state_replaced_entities(
        self, compile_state: ORMCompileState, mapper_entities: Sequence[_MapperEntity]
    ) -> None:
        ...
        self._process(
            compile_state, mapper_entities, not bool(compile_state.current_path)
        )
    ...
    @classmethod
    def _chop_path(
        cls, to_chop: _PathRepresentation, path: PathRegistry, debug: bool = False
...
class Load(_AbstractLoad):
    ...
    _traverse_internals = [
        ...
        ("context", visitors.InternalTraversal.dp_has_cache_key_list),
    ...
    def _adjust_for_extra_criteria(self, context: QueryContext) -> Load:
        ...
        def process(
            opt: _LoadElement, replacement_cache_key: CacheKey, orig_cache_key: CacheKey
        ) -> _LoadElement:
    ...
    def _process(
        ...
        for loader in self.context:
            ...
            loader.process_compile_state(
                self, compile_state, mapper_entities, reconciled_lead_entity, raiseerr
            )
...
class _WildcardLoad(_AbstractLoad):
    ...
    _traverse_internals = [
        ...
        ("local_opts", visitors.ExtendedInternalTraversal.dp_string_multi_dict),
    ]
    ...
    def _find_entity_basestring(
        self, entities: Iterable[_InternalEntityType[Any]], token: str, raiseerr: bool
