        ...
        if self.prop.uselist and state is None:
            ...
            if self.mapper._canload(state, allow_subtypes=True):
                ...
                raise exc.FlushError(
                    ...
                    "%(z)s or a polymorphic subclass of "
                    "this type."
                    % {"x": state.class_, "y": self.prop, "z": self.mapper.class_}
                )
    ...
    def per_state_dependencies(
        ...
        if self.post_update:
            ...
            if not isdelete:
                if childisdelete:
                    uow.dependencies.update(
                        [(child_action, after_save), (after_save, child_post_updates)]
                    )
                else:
                    uow.dependencies.update(
                        [
        ...
        for state in states:
            ...
            if history:
                for child in history.added:
                    if child is not None:
                        uowcommit.register_object(
                            child, cancel_delete=True, operation="add", prop=self.prop
                        )
                ...
                for child in history.deleted:
                    if not self.cascade.delete_orphan:
                        ...
                        uowcommit.register_object(
                            child, isdelete=True, operation="delete", prop=self.prop
                        )
...
class ManyToOneDP(DependencyProcessor):
    ...
    def _synchronize(
        self, state, child, associationrow, clearkeys, uowcommit, operation=None
    ):
...
class ManyToManyDP(DependencyProcessor):
            ...
            if history:
                for child in history.non_added():
                    ...
                    if not self._synchronize(
                        state, child, associationrow, False, uowcommit, "delete"
                    ):
                        continue
    ...
    def _synchronize(
        ...
        sync.populate_dict(
            child, self.mapper, associationrow, self.prop.secondary_synchronize_pairs
        )
