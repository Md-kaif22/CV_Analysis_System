...
class QueryableAttribute(
    ...
    def _memoized_attr__propagate_attrs(self) -> _PropagateAttrsType:
        ...
        return util.immutabledict(
            {"compile_state_plugin": "orm", "plugin_subject": self._parentmapper}
        )
    ...
    def __getattr__(self, key: str) -> Any:
        ...
        try:
            ...
            raise AttributeError(
                ...
                "has an attribute %r"
                % (type(self).__name__, type(self.comparator).__name__, self, key)
            ) from err
...
class AttributeImpl:
    ...
    def sethasparent(
        self, state: InstanceState[Any], parent_state: InstanceState[Any], value: bool
    ) -> None:
                ...
                if last_parent is not False and last_parent.key != parent_state.key:
                    if last_parent.obj() is None:
                        raise orm_exc.StaleDataError(
                            ...
                            % (state_str(state), state_str(parent_state), self.key)
                        )
    ...
    def pop(
        ...
        self.set(
            state, dict_, None, initiator, passive=passive, check_old=value, pop=True
        )
...
class ScalarObjectAttributeImpl(ScalarAttributeImpl):
    ...
    def fire_remove_event(
        ...
        if self.trackparent and value not in (None, PASSIVE_NO_RESULT, NO_VALUE):
...
class History(NamedTuple):
    ...
    * ``deleted`` - the collection of items that have been removed from the
    ...
    def from_scalar_attribute(
        cls, attribute: ScalarAttributeImpl, state: InstanceState[Any], current: Any
    ) -> History:
    ...
    @classmethod
    def from_collection(
        cls, attribute: CollectionAttributeImpl, state: InstanceState[Any], current: Any
    ) -> History:
        original = state.committed_state.get(attribute.key, _NO_HISTORY)
        if current is NO_VALUE:
            return cls((), (), ())
