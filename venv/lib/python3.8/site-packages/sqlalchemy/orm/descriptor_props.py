...
_CompositeAttrType = Union[
    str, "Column[_T]", "MappedColumn[_T]", "InstrumentedAttribute[_T]", "Mapped[_T]"
]
...
class CompositeProperty(
        ...
        def fset(instance: Any, value: Any) -> None:
            ...
            if value is None:
                ...
                for key, value in zip(
                    self._attribute_keys, self._composite_values_from_instance(value)
                ):
    ...
    def _setup_event_handlers(self) -> None:
        ...
        def insert_update_handler(
            mapper: Mapper[Any], connection: Connection, state: InstanceState[Any]
        ) -> None:
    ...
    def get_history(
        ...
        if has_history:
            return attributes.History(
                [self.composite_class(*added)], (), [self.composite_class(*deleted)]
            )
        else:
        ...
        if self.map_column:
            ...
            if self.key not in parent.persist_selectable.c:
                raise sa_exc.ArgumentError(
                    ...
                    "'%s' named '%s'"
                    % (self.name, parent.persist_selectable.description, self.key)
                )
            elif (
