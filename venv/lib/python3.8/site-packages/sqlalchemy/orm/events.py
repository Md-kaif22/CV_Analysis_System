...
class InstrumentationEvents(event.Events[InstrumentationFactory]):
    ...
    def _accept_with(
        cls,
        target: Union[InstrumentationFactory, Type[InstrumentationFactory]],
        identifier: str,
    ) -> Optional[Union[InstrumentationFactory, Type[InstrumentationFactory]]]:
        if isinstance(target, type):
            return _InstrumentationEventsHold(
                target
    ...
    def _listen(
        ...
        def remove(ref: ReferenceType[_T]) -> None:
            key = event.registry._EventKey(  # type: ignore [type-var]
                None, identifier, listen, instrumentation._instrumentation_factory
            )
            getattr(
...
class InstanceEvents(event.Events[ClassManager[Any]]):
    ...
    * the :class:`_orm.Mapper` class itself indicates listening for all
    ...
    def _accept_with(
        cls, target: Union[ClassManager[Any], Type[ClassManager[Any]]], identifier: str
    ...
    def refresh_flush(
        self, target: _O, flush_context: UOWTransaction, attrs: Optional[Iterable[str]]
    ) -> None:
...
class _EventsHold(event.RefCollection[_ET]):
    ...
    def __init__(
        self, class_: Union[DeclarativeAttributeIntercept, DeclarativeMeta, type]
    ) -> None:
        self.class_ = class_
...
class _InstanceEventsHold(_EventsHold[_ET]):
    ...
    class HoldInstanceEvents(
        _EventsHold.HoldEvents[_ET], InstanceEvents
        ...
        if not raw or not retval:
            ...
            def wrap(*arg: Any, **kw: Any) -> Any:
                if not raw and target_index is not None:
                    ...
                    arg[target_index] = arg[
                        target_index
                    ].obj()  # type: ignore [index] # noqa: E501
                if not retval:
                    fn(*arg, **kw)
...
class _MapperEventsHold(_EventsHold[_ET]):
    ...
    class HoldMapperEvents(
        _EventsHold.HoldEvents[_ET], MapperEvents
    ):  # type: ignore [misc] # noqa: E501
        pass
...
class SessionEvents(event.Events[Session]):
    ...
    def _accept_with(  # type: ignore [return]
        ...
        if isinstance(target, sessionmaker):
            ...
            return event.Events._accept_with(
                target, identifier
            )  # type: ignore [return-value] # noqa: E501
        ...
        **kw: Any,
        ...
        if is_instance_event:
            if not raw or restore_load_context:
                ...
                def wrap(
                    session: Session, state: InstanceState[_O], *arg: Any, **kw: Any
                ) -> Optional[Any]:
                    if not raw:
                        target = state.obj()
                        if target is None:
...
class AttributeEvents(event.Events[QueryableAttribute[Any]]):
    ...
    def _listen(  # type: ignore [override]
        ...
        if propagate:
            ...
            for mgr in manager.subclass_managers(
                True
            ):  # type: ignore [no-untyped-call] # noqa: E501
    ...
    def append(
        self, target: _O, value: _T, initiator: Event, *, key: EventConstants = NO_KEY
    ) -> Optional[_T]:
