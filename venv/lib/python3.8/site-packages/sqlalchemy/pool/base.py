...
class Pool(log.Identified, event.EventTarget):
    ...
    def _close_connection(
        ...
        self.logger.debug(
            "%s connection %r", "Hard-closing" if terminate else "Closing", connection
        )
        try:
            if terminate:
...
class _ConnectionRecord(ConnectionPoolEntry):
    ...
    def driver_connection(
        self
    ) -> Optional[Any]:  # type: ignore[override]  # mypy#4125  # noqa: E501
    ...
    def get_connection(self) -> DBAPIConnection:
        ...
        if self.dbapi_connection is None:
            ...
            self.__pool.logger.info(
                "Connection %r exceeded timeout; recycling", self.dbapi_connection
            )
            recycle = True
        ...
        try:
            if not fairy:
                ...
                fairy = _ConnectionFairy(
                    pool, dbapi_connection, connection_record, echo
                )
            ...
            if detach:
                ...
                if can_close_or_terminate_connection:
                    ...
                    pool._close_connection(
                        dbapi_connection, terminate=requires_terminate_for_close
                    )
...
class _ConnectionFairy(PoolProxiedConnection):
            ...
            try:
                if pool._pre_ping:
                    if not connection_is_fresh:
                        if fairy._echo:
                            pool.logger.debug(
                                "Pool pre-ping on connection %s", fairy.dbapi_connection
                            )
    ...
    def _reset(
        ...
        if pool._reset_on_return is reset_rollback:
            ...
            if self._echo:
                pool.logger.debug(
                    "Connection %s commit-on-return", self.dbapi_connection
                )
