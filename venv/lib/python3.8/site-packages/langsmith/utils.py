...
def _middle_copy(
    ...
    if isinstance(val, list):
        return [
            _middle_copy(item, memo, max_depth, _depth + 1) for item in val
...
class ContextThreadPoolExecutor(ThreadPoolExecutor):
    ...
    def map(
        ...
        *iterables: Iterable[Any],
        ...
        """
        contexts = [
            contextvars.copy_context() for _ in range(len(iterables[0]))
...
def get_api_url(api_url: Optional[str]) -> str:
    ...
    _api_url = api_url or cast(
        str, get_env_var("ENDPOINT", default="https://api.smith.langchain.com")
    )
