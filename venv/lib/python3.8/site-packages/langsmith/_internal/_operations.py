    ...
    for key, value in (
        ...
        acc_parts.append(
            (
                ...
                (None, valb, "application/json", {"Content-Length": str(len(valb))}),
            )
        )
    if op.attachments:
        for n, (content_type, valb) in op.attachments.items():
            ...
            acc_parts.append(
                (
                    ...
                    (None, valb, content_type, {"Content-Length": str(len(valb))}),
                )
            )
    return MultipartPartsAndContext(acc_parts, f"trace={op.trace_id},id={op.id}")
