...
try:
    ...
    st.register_type_strategy(
        pydantic.EmailStr, st.emails().filter(is_valid_email)
...
st.register_type_strategy(
    pydantic.IPvAnyAddress, st.ip_addresses()
)  # type: ignore[arg-type]
st.register_type_strategy(
...
def resolves(
    ...
    [Callable[..., st.SearchStrategy]], Callable[..., st.SearchStrategy]
]:  # type: ignore[type-arg]
...
def resolve_json(cls):  # type: ignore[no-untyped-def]
    try:
        ...
        inner = st.recursive(
            ...
            extend=lambda x: st.lists(x)
            | st.dictionaries(st.text(), x),  # type: ignore
        )
...
def resolve_conbytes(cls):  # type: ignore[no-untyped-def]  # pragma: no cover
    ...
    repeats = "{{{},{}}}".format(
        min_size - 2 if min_size > 2 else 0, max_size - 2 if (max_size or 0) > 2 else ""
    )
    if min_size >= 2:
