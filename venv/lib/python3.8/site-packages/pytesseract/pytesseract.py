...
class ALTONotSupported(EnvironmentError):
    def __init__(self):
        super().__init__("ALTO output not supported. Tesseract >= 4.1.0 required")
...
def run_tesseract(
    input_filename, output_filename_base, extension, lang, config="", nice=0, timeout=0
):
    cmd_args = []
...
def run_and_get_output(
    image, extension="", lang=None, config="", nice=0, timeout=0, return_bytes=False
):
    with save(image) as (temp_name, input_filename):
        kwargs = {
            "input_filename": input_filename,
        ...
        run_tesseract(**kwargs)
        return _read_output(
            f"{kwargs['output_filename_base']}{extsep}{extension}", return_bytes
        )
...
def image_to_alto_xml(image, lang=None, config="", nice=0, timeout=0):
    """
...
def image_to_data(
    ...
    return {
        Output.BYTES: lambda: run_and_get_output(*(args + [True])),
        Output.DATAFRAME: lambda: get_pandas_output(args + [True], pandas_config),
